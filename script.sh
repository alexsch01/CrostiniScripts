if ! grep -R 'sudo sysctl -w net.ipv6.conf.all.disable_ipv6=1' ~/.bashrc > /dev/null; then
        echo 'sudo sysctl -w net.ipv6.conf.all.disable_ipv6=1' >> ~/.bashrc
fi

if ! grep -R 'sudo sysctl -w net.ipv6.conf.default.disable_ipv6=1' ~/.bashrc > /dev/null; then
        echo 'sudo sysctl -w net.ipv6.conf.default.disable_ipv6=1' >> ~/.bashrc
fi
